...
export function ToolCard({ icon, titleKey, descKey, href, state = "available", progress }: ToolCardProps) {
  ...
  if (isDisabled) {
    return (
      <div
        className="block bg-white rounded-3xl p-6 shadow-[var(--shadow-medium)] transition-all duration-300 opacity-60 cursor-not-allowed"
      >
        {cardContent}
      </div>
    )
  ...
  return (
    <Link
      to={href}
      className="block bg-white rounded-3xl p-6 shadow-[var(--shadow-medium)] transition-all duration-300 hover:shadow-[var(--shadow-high)] hover:scale-[1.02] cursor-pointer"
    >
      {cardContent}
    </Link>
  )
